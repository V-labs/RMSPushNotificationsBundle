...
class WebFCMNotification implements OSNotificationServiceInterface
{
    ...
    public function send(MessageInterface $message)
    {
        ...
        if (count($message->getFCMIdentifiers()) == 1)
        {
            ...
            $this->logger->notice('Sending single WebFCM push notification with data: ' . json_encode($data));
        ...
        else
        {
            ...
            foreach ($chunks as $registrationIDs) {
                ...
                $this->logger->notice('Sending multiple WebFCM push notification with data: ' . json_encode($data));
                $this->responses[] = $client->post($this->apiURL, [
                    'headers' => $headers,
                    'body' => json_encode($data)
                ]);
